...
const CardDetectionApp = ({ params }) => {
  // State for resolved parameters
  const [resolvedParams, setResolvedParams] = useState(null);
  const [businessName, setBusinessName] = useState("");
  const [scanId, setScanId] = useState("");

  // Extract URL parameters from Next.js params (resolve for Next.js 15)
  useEffect(() => {
    const resolveParams = async () => {
      const resolved = await params;
      setResolvedParams(resolved);
      setBusinessName(resolved.businessname);
      setScanId(resolved.scanid);
    };
    resolveParams();
  }, [params]);

  // Validate scanId when resolved
  const isValidScanIdParam = resolvedParams ? isValidScanId(scanId) : false;

  // URL parameters state
  const [urlParams, setUrlParams] = useState({
    businessName: businessName,
    scanId: isValidScanIdParam ? scanId : null
  });
  ...
  useEffect(() => {
    // Set URL parameters when resolved
    if (resolvedParams) {
      console.log('🔍 URL Parameters from Next.js:', { businessName, scanId, isValidScanIdParam });
      setUrlParams({
        businessName: businessName,
        scanId: isValidScanIdParam ? scanId : null
      });
    }
  }, [businessName, scanId, isValidScanIdParam, resolvedParams]);
  ...
  useEffect(() => {
    ...
    // Only fetch merchant info if we have URL params loaded
    if (urlParams.businessName !== null || urlParams.scanId !== null) {
      fetchMerchantInfo();
    }
  }, [urlParams]); // Add urlParams as dependency
  ...
  const {
    ...
    clearDetectionTimeout,
    urlParams.scanId // Pass scanId to the hook
  );
  ...
  const startCardValidation = async () => {
    ...
    // Prevent starting if detection is already active
    if (detectionActive) {
      console.log("🚫 Cannot start validation - detection already active");
      return;
    }
    
    // Prevent starting if already in results or final phase
    if (currentPhase === "results" || currentPhase === "final_response" || finalOcrResults) {
      console.log("🚫 Cannot start validation - already in final state");
      return;
    }
    
    // Additional safety check for locked state
    if (lockedRef.current) {
      console.log("🚫 Cannot start validation - result already locked");
      return;
    }
  ...
  const startFrontSideDetection = async () => {
    ...
    // Prevent starting if detection is already active
    if (detectionActive) {
      console.log("🚫 Cannot start front scan - detection already active");
      return;
    }
    
    // Prevent starting if already in results or final phase
    if (currentPhase === "results" || currentPhase === "final_response" || finalOcrResults) {
      console.log("🚫 Cannot start front scan - already in final state");
      return;
    }
    
    // Additional safety check for locked state
    if (lockedRef.current) {
      console.log("🚫 Cannot start front scan - result already locked");
      return;
    }
  ...
  const startBackSideDetection = async () => {
    ...
    // Prevent starting if already in results or final phase
    if (currentPhase === "results" || currentPhase === "final_response" || finalOcrResults) {
      console.log("🚫 Cannot start back scan - already in final state");
      return;
    }
    
    // Additional safety check for locked state
    if (lockedRef.current) {
      console.log("🚫 Cannot start back scan - result already locked");
      return;
    }
    ...
    startCountdown(async () => {
      ...
      try {
        ...
        if (!stopRequestedRef.current && !lockedRef.current) {
          ...
          // Lock the state to prevent any further detection attempts
  ...
  const resetApplication = () => {
    ...
    clearAllTimeouts();
    lockedRef.current = false; // Reset locked state
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-700 to-black p-4 flex items-center justify-center">
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-md text-center">
          ...
          <p className="text-gray-600">Initializing card detection app...</p>
        </div>
      </div>
    );
  }

  // Show error if scanId is invalid
  if (!isValidScanIdParam) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-700 to-black p-4 flex items-center justify-center">
        <div className="bg-red-100 border border-red-400 text-red-700 px-6 py-4 rounded-lg max-w-md">
          <h2 className="text-xl font-bold mb-2">Invalid Scan ID</h2>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-700 to-black p-4 sm:p-4">
      <div className="container mx-auto max-w-4xl">
        <div className="text-center mb-4 sm:mb-8">
          {/* URL Info Display */}
          {/* <div className="mb-4 p-2 bg-blue-100 rounded-md text-sm text-blue-800">
            <strong>Business:</strong> {businessName} | <strong>Scan ID:</strong> {scanId}
          </div> */}
